...
Dagger::Dagger(Player *player, Orientation orientation, int damage) {
	type = PROJECTILE;
	this->player = player;
	...
	case FACING_UP:
		vel = Vector2D(0, -1);
		position = position.add(Vector2D(0, -height));
		break;
	case FACING_RIGHT:
		vel = Vector2D(1, 0);
		position = position.add(Vector2D(width, 0));
		break;
	case FACING_DOWN:
		vel = Vector2D(0, 1);
		position = position.add(Vector2D(0, height));
...
void Dagger::collisionWith(Entity *entity) {
...
void Dagger::update(double dt, Map *map, std::vector<Entity *> entities) {
...
BoundingBox Dagger::hasMapCollision(Map *map) {
...
void Dagger::render(double edt) {
	if (!Rogue::rogue) {
		Rogue::rogue = al_load_bitmap("resources/graphics/rogue.png");
	}
	...
	switch (orientation) {
	case FACING_UP:
		al_draw_tinted_scaled_rotated_bitmap_region(Rogue::rogue, width, 0, width, height, al_map_rgb(255, 255, 255),
		        width / 2, height / 2,
