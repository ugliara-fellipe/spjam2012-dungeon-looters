...
ALLEGRO_SAMPLE *Boots::sound = 0;

Boots::Boots():Item(BOOTS_OF_SPEED) {
}

Boots::~Boots() {
}

void Boots::render(double edt) {
	if (!Item::items) {
...
void Boots::action( Map *map, std::vector<Entity *> entities, Player **player, Player* eu) {
	Player *use;
	if(eu == player[0]) {
		use = player[1];
	} else {
	...
	use->playerType = ROGUE;
	target = use;
}

ParticleSystem *Boots::getUseParticles() {
	Particle *particle = new Particle(12, 32, 32, 0.03, 1.0, 0.0, 0.4);
	ParticleSystem *particleSystem = new ParticleSystem(particle, RADIAL_ACCEL_IN, target, 120, 0, 0.3);

	return particleSystem;
}

void Boots::playUseSound() {
	if (!sound) {
